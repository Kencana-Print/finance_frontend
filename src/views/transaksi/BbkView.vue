...
import { bbkApi, type BbkRow, type BbkDetailRow } from "@/api/transaksi/bbkApi";
import { exportBbk, exportBbkDetail } from "@/utils/exportExcel";
import {
  IconBuildingBank,
  IconPrinter,
  IconFileSpreadsheet,
...
// ── Periode ───────────────────────────────────────────────────────────
const STORAGE_KEY = "finance_periode_bbk"; // ganti per modul: bkk, bbk, dll
...
// Tambah watcher untuk simpan setiap kali berubah:
...
// ── Data ──────────────────────────────────────────────────────────────
const items = ref<BbkRow[]>([]);
const detailItems = ref<BbkDetailRow[]>([]);
const isLoading = ref(false);
const selected = ref<BbkRow[]>([]);
...
const headers = [
  ...
  { key: "Link", title: "Link", width: "160px" },
...
const loadData = async () => {
  ...
  try {
    const [master, detail] = await Promise.all([
      bbkApi.getBrowse(startDate.value, endDate.value),
      bbkApi.getBrowseDetail(startDate.value, endDate.value),
    ]);
    items.value = master;
...
const validateAction = (action: "ubah" | "hapus" | "cetak"): boolean => {
  ...
  // Delphi: Link bisa dari BON atau PJT
...
// ── Aksi ──────────────────────────────────────────────────────────────
const onBaru = () => router.push({ name: "BbkCreate" });
const onUbah = () => {
...
const onCetak = () => {
  ...
  window.open(
    `/transaksi/bbk/print/${encodeURIComponent(selectedItem.value.Nomor)}`,
    "_blank",
  );
};
...
const confirmDelete = async () => {
  ...
  try {
    await bbkApi.delete(selectedItem.value.Nomor);
...
// ── Export ────────────────────────────────────────────────────────────
const doExport = () => exportBbk(items.value, startDate.value, endDate.value);
const doExportDetail = () =>
  exportBbkDetail(detailItems.value, startDate.value, endDate.value);

// ── Row props ─────────────────────────────────────────────────────────
const rowPropsFn = (data: any) => {
  const row = data.item?.raw || data.item;
  let cls = "";
  if (row.Link) cls += " row-link";
