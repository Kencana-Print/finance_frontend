...
import { bkkApi, type BkkRow, type BkkDetailRow } from "@/api/transaksi/bkkApi";
import { exportBkk, exportBkkDetail } from "@/utils/exportExcel";
import {
  IconTransferOut,
  IconPrinter,
  IconFileSpreadsheet,
...
// ── Periode ───────────────────────────────────────────────────────────
const STORAGE_KEY = "finance_periode_bkk";
...
// ── Data ──────────────────────────────────────────────────────────────
const items = ref<BkkRow[]>([]);
const detailItems = ref<BkkDetailRow[]>([]);
const isLoading = ref(false);
const selected = ref<BkkRow[]>([]);
...
const loadData = async () => {
  ...
  try {
    const [master, detail] = await Promise.all([
      bkkApi.getBrowse(startDate.value, endDate.value),
      bkkApi.getBrowseDetail(startDate.value, endDate.value),
    ]);
    items.value = master;
...
// ── Detail per baris (untuk expanded row) ─────────────────────────────
...
const validateAction = (action: "ubah" | "hapus" | "cetak"): boolean => {
  ...
  if (r.Kasbon && action !== "cetak") {
    const msg =
      action === "ubah"
        ? "BKK terbentuk otomatis dari kasbon. Tidak bisa diubah."
        : "BKK terbentuk otomatis dari kasbon. Tidak bisa dihapus.";
    toast.warning(msg);
    return false;
  }
...
// ── Aksi ──────────────────────────────────────────────────────────────
const onBaru = () => router.push({ name: "BkkCreate" });
const onUbah = () => {
...
const onCetak = () => {
  ...
  if (selectedItem.value.Kasbon) {
    toast.warning("BKK dari kasbon. Silakan cetak di Penyelesaian Kasbon.");
    return;
  }
  window.open(
    `/transaksi/bkk/print/${encodeURIComponent(selectedItem.value.Nomor)}`,
    "_blank",
  );
};
...
const confirmDelete = async () => {
  ...
  try {
    await bkkApi.delete(selectedItem.value.Nomor);
...
// ── Export ────────────────────────────────────────────────────────────
const doExport = () => exportBkk(items.value, startDate.value, endDate.value);
const doExportDetail = () =>
  exportBkkDetail(detailItems.value, startDate.value, endDate.value);
...
<template>
  ...
  >
    ...
    <template #extra-actions>
      ...
      <v-btn size="small" variant="tonal" @click="onCetak">
        <template #prepend
