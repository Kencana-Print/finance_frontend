...
import { bkmFormApi, type BkmFormDetail } from "@/api/transaksi/bkmFormApi";
...
onMounted(async () => {
  ...
  try {
    const [acc, cc, accAll] = await Promise.all([
      bkmFormApi.getAccountOptions(authStore.userCabang || "P01"),
      bkmFormApi.getCostCenterOptions(),
      bkmFormApi.getAccountAll(),
    ]);
    ...
    if (!isEdit.value) {
      ...
      const d = await bkmFormApi.getDetailForm(
...
const openDcModal = async (idx: number) => {
  ...
  dcOptions.value = await bkmFormApi.getDcOptions(d.cckode);
...
const validateSave = () => {
  ...
  if (totalBkm.value === 0) {
    toast.warning("Total BKM kosong. Tidak bisa disimpan.");
    return;
  }

  for (const d of filled) {
    // Delphi: skip validasi jika SALDO atau PENYESUAIAN
...
const confirmSave = async () => {
  isSaving.value = true;
  try {
    const res = await bkmFormApi.save({ ...form.value, isEdit: isEdit.value });
...
const cetakBkm = () => {
  window.open(
    `/transaksi/bkm/print/${encodeURIComponent(savedNomor.value)}`,
    "_blank",
  );
  showPrintDialog.value = false;
  router.push({ name: "BkmBrowse" });
};
const skipCetak = () => {
  showPrintDialog.value = false;
  router.push({ name: "BkmBrowse" });
  ...
  >
    ...
    <template #left-column>
      <div
        ...
        "
      >
        <div class="section-title">Informasi BKM</div>

        <!-- Nomor -->
        <div class="field-row">
          <label class="field-lbl">Nomor BKM</label>
        ...
        <!-- Keterangan (free text, bukan dropdown) -->
        <div class="field-row">
        ...
        <div class="total-box mt-3">
          <span class="total-lbl">Total BKM</span>
          <span class="total-val">{{ fmt(totalBkm) }}</span>
        </div>
      </div>
    </template>
  ...
  <v-dialog v-model="showPrintDialog" max-width="380" persistent>
    <v-card rounded="lg">
      ...
      <v-card-text class="pa-4 pt-2" style="font-size: 12px">
        Nomor BKM: <strong>{{ savedNomor }}</strong
        ><br />
        Ingin mencetak BKM sekarang?
      </v-card-text>
      <v-card-actions class="pa-3">
        <v-btn variant="text" @click="skipCetak">Tidak</v-btn>
        <v-spacer />
        <v-btn color="primary" variant="flat" @click="cetakBkm">
          <template #prepend
