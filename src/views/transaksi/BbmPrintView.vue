...
import { bbmFormApi } from "@/api/transaksi/bbmFormApi";
...
onMounted(async () => {
  try {
    ...
    data.value = await bbmFormApi.getPrintData(nomor);
  ...
  const convert = (num: number): string => {
    ...
    if (num < 1_000_000_000)
      return (
        ...
        " JUTA " +
        convert(num % 1_000_000)
      );
    return (
      convert(Math.floor(num / 1_000_000_000)) +
      " MILIAR " +
      convert(num % 1_000_000_000)
    );
  };
  if (n === 0) return "NOL";
  ...
  <div v-else-if="data" class="print-page">
    ...
    <div class="header-wrap">
      <div class="header-left">
        <div class="judul">BUKTI BANK MASUK</div>
        <table class="info-tbl">
          ...
          <tr>
            ...
            <td class="info-key">Diterima dari</td>
            <td class="info-sep">:</td>
            <td class="info-val">{{ data.penerima }}</td>
          </tr>
        </table>
      </div>
